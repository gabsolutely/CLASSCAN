...
1. **Edge-Based Head and Occupancy Detection:**
   - Detects people in classroom seating using a lightweight single-class **"head"** (head-and-shoulders) TFLite object detection model (`classcan_head_v1.tflite` — custom Keras multi-scale MobileNetV2 head detector with 3-head FPN, occupancy-based target encoding).
